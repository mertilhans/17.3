# 🐚 MiniShell Projesi - Derinlemesine Teknik Analiz ve Kapsamlı Kılavuz
...
## Bölüm 1: Proje Mimarisi ve Genel Veri Akışı
...
1.  **Girdi `(char *)`**: `readline` ile alınan ham metin.
...
5.  **➡️ Modül 4: EXECUTOR**
    * **İşlem**: Genişletilmiş komut listesini alır ve çalıştırır.
    * `pipe()` oluşturulur.
    * **Komut 1 için `fork()`**: Child 1, `echo`'yu çalıştırır, çıktısını `pipe`'ın yazma ucuna yönlendirir.
    * **Komut 2 için `fork()`**: Child 2, `pipe`'ın okuma ucunu `stdin` olarak ayarlar, `output.txt` dosyasını `stdout` olarak açar ve `wc -c` komutunu çalıştırır.
    * **Parent Process**: Her iki child process'in de bitmesini bekler.

---

## Bölüm 2: Ana Döngü ve Başlatma (`shell.c`)
...
## Bölüm 4: Modül 2: Parser (`parser*.c`)
...
## Bölüm 5: Modül 3: Expander (`expander*.c`)
...
-   **`char *handle_dollar(char *str, int *i, t_env *env_list)`**:
    ...
    -   **Adım Adım Çalışması**:
        ...
        2.  `str[*i]`'nin ne olduğuna bakar:
            ...
            * `search_variable`: Bu isimle önce `env_list`'te (`search_env_variable`), bulunamazsa `export_list`'te (`search_export_variable`) arama yapar.
        3.  Bulunan değeri (`value`) veya bulunamazsa boş string'i (`""`) döndürür.

---

## Bölüm 6: Modül 4: Executor (`execute*.c`)
...
## Bölüm 7: Yardımcı Modüller ve Fonksiyonlar

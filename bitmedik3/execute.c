...
char *find_executable(char *cmd)
{
    ...
    // Eğer komut absolute veya relative path içeriyorsa
    if (ft_strchr(cmd, '/'))
    {
    ...
    if (!path || !*path)
    {
        // PATH yoksa /bin, /usr/bin gibi standart yerlerde ara
        char *default_paths[] = {"/bin", "/usr/bin", "/usr/local/bin", NULL};
        int i = 0;
        
        while (default_paths[i])
        {
            result = check_cmd_in_dir(default_paths[i], 
                     ft_strlen(default_paths[i]), cmd);
            if (result)
                return result;
            i++;
        }
...
int heredoc_fd_error(t_parser *cmd_list, t_exec_data *data, t_parser *current_cmd)
{
    ...
    // Signal'ları restore et ÖNCE
    setup_interactive_signals();
    
    // Stdout'u düzgün restore et
    if (data->original_stdin != -1)
    {

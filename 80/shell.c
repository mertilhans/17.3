...
/*   Updated: 2025/08/10 16:42:20 by merilhan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	process_command_line(char *line, t_env **env_list, char **env)
{
	...
	if (!tokens)
		return (get_last_exit_status());                               
			// FIX: Mevcut exit status'u koru
	cmd_list = parse_tokens(tokens, *env_list, get_last_exit_status());
		// FIX: Güncel exit status'u geçir
	if (!cmd_list)
		return (get_last_exit_status()); // FIX: Mevcut exit status'u koru
	...
	// FIX: Sadece exit builtin için shell'den çık, diğer error'lar için değil
	set_last_exit_status(exit_status);
	return (0); // Normal case'lerde shell loop'u devam ettir
}

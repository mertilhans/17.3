...
char	*ft_readline_loop(t_reader *r)
{
	...
	// FIX: SIGINT ile interrupt edildiğinde de NULL return et (EINTR check)
	if (n <= 0 && r->pos == 0)
	{
		gc_free(r->buf);
		return (NULL);
...
char	*readline_loop(t_heredoc_buffer *buf, const char *delimiter)
{
	...
	while (1)
	{
		...
		if (!buf->line)
		{
			...
			setup_interactive_signals();
			return (NULL); // FIX: SIGINT durumunda NULL return et
		}
...
char	*read_single_heredoc_block(char *delimiter)
{
	...
	if (result)
		...
		return NULL; // FIX: SIGINT durumunda NULL return et, buf.content değil
}
		...
		int exit_status)
{
	...
	while (current_redir)
	{
		if (current_redir->type == REDIR_HEREDOC)
		{
			...
			if (ft_h_built_expand(current_redir, data, env_list, exit_status) ==
				-1)
				return (-1);
		}
...
int	ft_builtin_call_2(t_parser *cmd, t_env **env_list) // değişiklik yaptım
{
...
int	heredoc_handle(t_parser *cmd_list, t_exec_data *data, t_env *env_list)
{
	...
	while (current_cmd)
	{
		...
		{
			set_last_exit_status(130);
				// FIX: Heredoc hatası/iptal durumunda exit status 130 set et
...
{
	data->return_status = 0; // buraya return
...
int	execute_cmds(t_parser *cmd_list, char **env, t_env **env_list)
{
	...
	if (heredoc_handle(cmd_list, &data, *env_list))
	{
		...
		return (get_last_exit_status());
			// FIX: Heredoc error durumunda direkt çık
	}
